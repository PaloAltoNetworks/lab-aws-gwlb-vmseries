...
- [3. Lab Steps](#3-lab-steps)
  ...
  - [3.3. new section](#33-new-section)
  - [3.4. Check Marketplace Subscriptions](#34-check-marketplace-subscriptions)
  - [3.5. Launch CloudShell](#35-launch-cloudshell)
  - [3.6. Search Available VM-Series Images (AMIs)](#36-search-available-vm-series-images-amis)
  - [3.7. Download Terraform](#37-download-terraform)
  - [3.8. Clone Deployment Git Repository](#38-clone-deployment-git-repository)
  - [3.9. Update Deployment Values in tfvars](#39-update-deployment-values-in-tfvars)
  - [3.10. Apply Terraform](#310-apply-terraform)
  - [3.11. Inspect deployed resources](#311-inspect-deployed-resources)
    - [3.11.1. Get VM-Series instance screenshot](#3111-get-vm-series-instance-screenshot)
    - [3.11.2. Check VM-Series instance details](#3112-check-vm-series-instance-details)
    - [3.11.3. Check cloudwatch bootstrap logs](#3113-check-cloudwatch-bootstrap-logs)
  - [3.12. Verify Bootstrap in Panorama](#312-verify-bootstrap-in-panorama)
  - [3.13. Access VM-Series Management](#313-access-vm-series-management)
  - [3.14. Check bootstrap logs](#314-check-bootstrap-logs)
  - [3.15. Fix GWLB Health Probes](#315-fix-gwlb-health-probes)
  - [3.16. Inbound Traffic Flows to App Spoke VPCs](#316-inbound-traffic-flows-to-app-spoke-vpcs)
    - [3.16.1. Update App1 Spoke VPC networking for Inbound inspection with GWLB](#3161-update-app1-spoke-vpc-networking-for-inbound-inspection-with-gwlb)
    - [3.16.2. Update App2 Spoke VPC networking for Inbound inspection with GWLB](#3162-update-app2-spoke-vpc-networking-for-inbound-inspection-with-gwlb)
    - [3.16.3. Test Inbound Traffic to Spoke Web Apps](#3163-test-inbound-traffic-to-spoke-web-apps)
    - [3.16.4. Test Outbound Traffic from App1 Spoke Instance](#3164-test-outbound-traffic-from-app1-spoke-instance)
    - [3.16.5. Check Inbound Traffic Logs](#3165-check-inbound-traffic-logs)
    - [3.16.6. Check Outbound Traffic Logs](#3166-check-outbound-traffic-logs)
  - [3.17. Outbound and East / West (OBEW) Traffic Flows](#317-outbound-and-east--west-obew-traffic-flows)
    - [3.17.1. Update App1 Spoke VPC for OB/EW routing with TGW](#3171-update-app1-spoke-vpc-for-obew-routing-with-tgw)
    - [3.17.2. Update App2 Spoke VPC for OB/EW routing with TGW](#3172-update-app2-spoke-vpc-for-obew-routing-with-tgw)
    - [3.17.3. Update Transit Gateway (TGW) Route Tables](#3173-update-transit-gateway-tgw-route-tables)
    - [3.17.4. Update Security VPC networking for OB/EW with GWLB](#3174-update-security-vpc-networking-for-obew-with-gwlb)
  - [3.18. Test Traffic Flows](#318-test-traffic-flows)
    - [3.18.1. Test Outbound Traffic from App1 Spoke Instances](#3181-test-outbound-traffic-from-app1-spoke-instances)
    - [3.18.2. Test Inbound Web Traffic to App1 Spoke and App2 Spoke](#3182-test-inbound-web-traffic-to-app1-spoke-and-app2-spoke)
    - [3.18.3. Test E/W Traffic from App1 Spoke Instance to App2 Spoke Instance](#3183-test-ew-traffic-from-app1-spoke-instance-to-app2-spoke-instance)
  - [3.19. GWLBE / Sub-Interface associations](#319-gwlbe--sub-interface-associations)
    - [3.19.1. Configure Zones in Panorama](#3191-configure-zones-in-panorama)
    - [3.19.2. Configure Sub-Interfaces in Panorama](#3192-configure-sub-interfaces-in-panorama)
    - [3.19.3. Create associations from GWLB Endpoints](#3193-create-associations-from-gwlb-endpoints)
    - [3.19.4. Create Zone-Based policies for sub-interfaces](#3194-create-zone-based-policies-for-sub-interfaces)
  - [3.20. Overlay Routing](#320-overlay-routing)
    - [3.20.1. Create Public Interfaces for VM-Series](#3201-create-public-interfaces-for-vm-series)
    - [3.20.2. Associate the EIPs to the Public Interfaces](#3202-associate-the-eips-to-the-public-interfaces)
    - [3.20.3. Configure Networking and Policies in Panorama](#3203-configure-networking-and-policies-in-panorama)
    - [3.20.4. Enable Overlay mode](#3204-enable-overlay-mode)
    - [3.20.5. Test Outbound Traffic](#3205-test-outbound-traffic)
...
## 3.4. Check Marketplace Subscriptions
...
- Notify lab instructor if you are still having issues

---

## 3.5. Launch CloudShell
...
## 3.6. Search Available VM-Series Images (AMIs)
...
## 3.8. Clone Deployment Git Repository 
...
## 3.9. Update Deployment Values in tfvars
...
## 3.11. Inspect deployed resources
...
---
### 3.11.1. Get VM-Series instance screenshot
...
### 3.11.2. Check VM-Series instance details
...
### 3.11.3. Check cloudwatch bootstrap logs
...
## 3.12. Verify Bootstrap in Panorama
...
## 3.13. Access VM-Series Management
...
## 3.16. Inbound Traffic Flows to App Spoke VPCs
...
### 3.16.1. Update App1 Spoke VPC networking for Inbound inspection with GWLB
...
### 3.16.2. Update App2 Spoke VPC networking for Inbound inspection with GWLB
...
###  3.16.3. Test Inbound Traffic to Spoke Web Apps
...
###  3.16.4. Test Outbound Traffic from App1 Spoke Instance
...
###  3.16.5. Check Inbound Traffic Logs
...
###  3.16.6. Check Outbound Traffic Logs
...
## 3.17. Outbound and East / West (OBEW) Traffic Flows

- The deployed topology does not have all of the AWS routing in place for a working GWLB topology and you must fix it!
- Refer to the diagram and try to resolve before looking at the specific steps
- We will work from left to right on the diagram

### 3.17.1. Update App1 Spoke VPC for OB/EW routing with TGW
...
### 3.17.2. Update App2 Spoke VPC for OB/EW routing with TGW
...
### 3.17.3. Update Transit Gateway (TGW) Route Tables
...
### 3.17.4. Update Security VPC networking for OB/EW with GWLB
...
###  3.18.1. Test Outbound Traffic from App1 Spoke Instances
...
###  3.18.2. Test Inbound Web Traffic to App1 Spoke and App2 Spoke
...
### 3.18.3. Test E/W Traffic from App1 Spoke Instance to App2 Spoke Instance
...
## 3.19. GWLBE / Sub-Interface associations
...
### 3.19.1. Configure Zones in Panorama
...
### 3.19.2. Configure Sub-Interfaces in Panorama
...
### 3.19.3. Create associations from GWLB Endpoints
...
### 3.19.4. Create Zone-Based policies for sub-interfaces
...
### 3.20.1. Create Public Interfaces for VM-Series
...
### 3.20.2. Associate the EIPs to the Public Interfaces
...
### 3.20.3. Configure Networking and Policies in Panorama
