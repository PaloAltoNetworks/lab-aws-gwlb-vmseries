...
> &#8505; **Running on a shared / standard AWS account (not QwikLabs)?** Follow the shared-account notes called out in each step. You authenticate with your own SSO/CLI credentials, bring your own SSH key, and set a unique `prefix_name_tag` so your resources don't collide with other students in the same account.

- Authenticate to the AWS account with your assigned credentials (QwikLabs console, or your SSO/CLI profile for a shared account)
- Verify your selected region. **The Panorama deployment region is `us-west-2`** — make sure your console/CLI is pointed there for the Panorama steps
...
### 4.1.1. Find or Create your SSH Key Pair
...
> &#9888; **EC2 key pairs are region-specific.** A key created in one region does not exist in another. Create / record your key in the region you are deploying into (Panorama = `us-west-2`).

**Shared / standard account — create your own key pair:**

- EC2 Console (region `us-west-2`) -> Key pairs -> **Create key pair**
- Give it a unique name, e.g. `yourname-gwlb-key`, and download the `.pem`/`.ppk`
- Record this name — you will put it in the Terraform `tfvars` in step 4.7

**QwikLabs — use the auto-generated key:**
...
> &#9888; **Shared / standard account: SKIP this section.** The `awsstudent` user and its `default_policy` Deny are QwikLabs-only. Your SSO role already has the needed permissions.
...
> &#9888; **Shared / standard account: you can SKIP CloudShell** and run everything from your **local machine** (VS Code terminal, macOS or WSL) using your SSO/CLI profile (`export AWS_PROFILE=<your-profile>`). Install Terraform locally per §4.6. Use CloudShell only if you prefer an in-browser shell. Either way, make sure your shell is authenticated and pointed at the correct region for each step (Panorama = us-west-2, VM-Series = us-east-1).
...
> &#8505; **Shared / standard account — edit `terraform.tfvars` before applying.** Open `terraform/panorama/terraform.tfvars` and set:
> - `panorama_ssh_key_name` = the key pair name you created in 4.1.1 (must exist in `us-west-2`). *(Leave `""` only on QwikLabs — it will auto-detect the `qwikLABS*` key.)*
> - `prefix_name_tag` = a short unique prefix for you, e.g. your initials + `-` (such as `"ad-"`). This keeps your VPC, TGW, IAM role, etc. from colliding with other students in the shared account.
>
> The AMI (`panorama_ami_id`) and region are already set for this account — no change needed.
...
- Authenticate with the Panorama admin credentials provided by your instructor (the pre-baked lab image ships with set credentials). *(On QwikLabs these were in `aws-gwlb-lab-secrets.txt`.)*
...
- Configure a SW Firewall License Bootstrap Definition
  - Name: `aws-gwlb-lab`
  - Auth Code: the VM-Series **flex-credit auth code** provided by your instructor (from the CSP/flex-credit pool). *(On QwikLabs this was in `aws-gwlb-lab-secrets.txt`.)*
...
Most of the bootstrap parameters and environment-specific variable values have already been prepped in the Terraform code for this deployment. You will need to set your `auth-key`, and (on a shared account) your SSH key and prefix.
...
> &#9888; **Shared / standard account — edit `security-vpc-east1.auto.tfvars` first.** Open it and set:
> - `vmseries_ssh_key_name` = your key pair name. **It must exist in `us-east-1`** (the VM-Series region) — key pairs are per-region, so create one in us-east-1 even if you already made one in us-west-2 for Panorama. *(Leave `""` only on QwikLabs.)*
...
> &#8505; The `auth-key` is the **VM-Series bootstrap parameter generated by *your* Panorama** in step 4.8 (Panorama → SW Firewall Licenses → License Manager → **Show Bootstrap Parameters**). It is not in a secrets file — copy it from that screen. (On QwikLabs it was pre-generated in `aws-gwlb-lab-secrets.txt`.)
...
```
- Use the VM-Series local credentials provided by your instructor for the VM-Series instances. These are the same credentials you used for Panorama. *(On QwikLabs these were in `aws-gwlb-lab-secrets.txt`.)*
