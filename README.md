...
- [4. Lab Steps](#4-lab-steps)
  - [4.1. Initialize Lab](#41-initialize-lab)
    - [4.1.1. SSH Key Pairs](#411-ssh-key-pairs)
  - [4.2. Update IAM Policies](#42-update-iam-policies)
  - [4.3. Check Marketplace Subscriptions](#43-check-marketplace-subscriptions)
  - [4.4. Set Up Your Terraform Execution Environment](#44-set-up-your-terraform-execution-environment)
...
Throughout this guide, the default steps assume a **QwikLabs** account. On a shared/standard account, follow the collapsed "shared / standard account" notes in each step instead:
...
Either way, this lab's Terraform **mints its own SSH key pairs** (see 4.1.1), and you **supply your own Software NGFW deployment-profile auth code** (see 4.7.1) - neither of those differs by account type anymore.

</details>
  
### 4.1.1. SSH Key Pairs

> &#8505; Every EC2 instance is associated with an SSH key pair, the default method of initial interactive login. With successful bootstrapping you should not need to connect to the VM-Series directly, but it is good to keep the key for emergency access. The same key is also used for the test web server instances.

This lab's Terraform **mints its own SSH key pair in each stack** - you do not create or download one. With the `*_ssh_key_name` variable left at its default (`""`), each stack generates a key pair, registers it in the deployment region, and writes the private key (mode `0600`, ready for `ssh -i`) next to the code:

- `terraform/panorama/panorama-ssh-key.pem` - Panorama (`us-west-2`)
- `terraform/vmseries/vmseries-ssh-key.pem` - VM-Series and the test web servers (`us-east-1`)

For example: `ssh -i panorama-ssh-key.pem admin@<panorama-eip>`. Both `.pem` files are gitignored and are never committed.

> &#9888; **EC2 key pairs are region-specific**, which is why each stack mints its own key in its own region.

<details>
<summary><b>⚙️ Bring your own key pair instead (optional)</b></summary>

To use an existing key pair rather than the minted one, set `panorama_ssh_key_name` (in `terraform/panorama/terraform.tfvars`) and/or `vmseries_ssh_key_name` (in `terraform/vmseries/security-vpc-east1.auto.tfvars`) to a key pair name that **already exists in that stack's region** (Panorama = `us-west-2`, VM-Series = `us-east-1`). Terraform then uses your key and skips minting.
...
## 4.4. Set Up Your Terraform Execution Environment

You need a shell with **Terraform, git, and the AWS CLI** plus credentials for the lab account to run the deployments from. The **recommended** path is the lab builder EC2 instance; running locally or in CloudShell also works.

> &#8505; Wherever you run Terraform from does **not** affect where the lab resources deploy. The tfvars pin the regions (Panorama in `us-west-2`, VM-Series and spokes in `us-east-1`), so the resources land there no matter where the runner lives.

### Recommended: the lab builder EC2

A small CloudFormation template (`builder/builder.cfn.yaml`) stands up an Amazon Linux 2023 instance with Terraform, git, and the AWS CLI preinstalled, this repo already cloned, an **IAM instance role** (no static keys), and **SSM Session Manager** access (no SSH key, no inbound ports). See [`builder/README.md`](builder/README.md) for the full walkthrough.

- Launch it in **`us-west-2`** (the Panorama region) for VPC/vCPU quota headroom. The builder creates its own small dedicated VPC, and on a QwikLabs account `us-east-1` is already crowded with the lab VPCs and the VM-Series compute.
- Connect with **EC2 > Instances > `gwlb-lab-builder` > Connect > Session Manager**, then `sudo su - ec2-user && cd lab-aws-gwlb-vmseries`.
- Because the repo is already cloned and Terraform is already installed, you can **skip the clone/install in 4.6** and go straight to 4.7 (run `terraform version` to confirm).

<details>
<summary><b>⚙️ Run locally instead (optional)</b></summary>

If you already have the AWS CLI and Terraform set up, run everything from your **local machine** (VS Code terminal, macOS or WSL) using your SSO/CLI profile (`export AWS_PROFILE=<your-profile>`). Install Terraform per 4.6. Make sure your shell is authenticated and pointed at the correct region for each step.

</details>

<details>
<summary><b>⚙️ Use AWS CloudShell instead (not recommended)</b></summary>

CloudShell works but has real limitations for this lab:
- Its home directory is only **1 GB**, which is too small once both stacks download their Terraform providers.
- Its storage is **per-region**, so switching regions mid-lab silently drops you into a fresh, empty environment - which is how you end up with duplicate resources or lost state.

If you still want to use it:
- *Verify you are in the assigned region!*
- Search for `cloudshell` in the top search bar, close the intro screen, and allow a few moments for it to initialize.
- CloudShell uses the same IAM role as your authenticated user and has git and the AWS CLI preinstalled. Anything saved under `/home/cloudshell-user` persists across sessions (within that region).
...
> &#8505; **Using the builder EC2 (4.4)?** The repo is already cloned to `~/lab-aws-gwlb-vmseries` and Terraform is already installed - skip ahead to the verify step below, then continue to 4.7.

If you are running locally or in CloudShell, clone the repo and install Terraform. The command below clones this repository and runs a script that installs Terraform into your environment:
...
Or on a local machine, follow instructions from HashiCorp:
