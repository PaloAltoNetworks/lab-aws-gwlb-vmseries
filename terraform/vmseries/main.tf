...
module "vmseries" {
  ...
  region              = var.region
  prefix_name_tag     = var.prefix_name_tag
  ssh_key_name        = local.vmseries_ssh_key_name
  ...
  {
    name    = "vmseries01"
    name_tag = "${var.prefix_name_tag}vmseries01"
    fw_tags = {}
  ...
  {
    name        = "vmseries02"
    name_tag    = "${var.prefix_name_tag}vmseries02"
    fw_tags = {}
...
# Match the student's OWN Panorama deployment by its prefixed Name tag. (Was filtered by
# amazon-side-asn=65200, which multi-matches on a shared account where many TGWs use that ASN.)

data "aws_ec2_transit_gateway" "us-west-2-tgw" {
  provider = aws.peer
  filter {
    name   = "tag:Name"
    values = ["${var.prefix_name_tag}tgw-us-west-2"]
  }
  filter {
    name   = "state"
    values = ["available"]
...
data "aws_ec2_transit_gateway_route_table" "us-west-2-tgw-rt" {
  ...
  filter {
    name   = "tag:Name"
    values = ["${var.prefix_name_tag}from-us-east-1-tgw-peer"]
  }
}
...
data "aws_ec2_transit_gateway_route_table" "us-west-2-spoke-rt" {
  ...
  filter {
    name   = "tag:Name"
    values = ["${var.prefix_name_tag}from-spoke-vpcs"]
  }
}
data "aws_ec2_transit_gateway_attachment" "us-west-2-managment-vpc" {
  ...
  filter {
    name   = "tag:Name"
    values = ["${var.prefix_name_tag}panorama-vpc"]
  }
  filter {
    name   = "state"
    values = ["available"]
...
resource "aws_ec2_transit_gateway_peering_attachment" "us-west-2-to-us-east1" {
  ...
  tags = {
    Name = "${var.prefix_name_tag}us-west-2-tgw-peer"
  }
}
...
resource "aws_ec2_transit_gateway_peering_attachment_accepter" "us-east1-from-us-west-2" {
  ...
  tags = {
    Name = "${var.prefix_name_tag}us-east-1-tgw-peer"
  }
}
...
resource "aws_instance" "spoke1_ec2_az1" {
  ...
  key_name               = local.vmseries_ssh_key_name
  ...
  root_block_device {
    ...
    volume_type           = "gp3" # gp3 required by account SCP (gp2 denied)
  }
...
resource "aws_vpc_endpoint" "spoke1_ssm" {
  ...
  tags                = merge(var.global_tags, { "Name" = "${var.prefix_name_tag}spoke1-ssm-endpoint"})
}
...
resource "aws_vpc_endpoint" "spoke1_kms" {
  ...
  private_dns_enabled = true
  security_group_ids = [module.spoke1_vpc.security_group_ids["web-server-sg"]]
  tags                = merge(var.global_tags, { "Name" = "${var.prefix_name_tag}spoke1-kms-endpoint"})
}
...
resource "aws_vpc_endpoint" "spoke1_ec2messages" {
  ...
  private_dns_enabled = true
  security_group_ids = [module.spoke1_vpc.security_group_ids["web-server-sg"]]
  tags                = merge(var.global_tags, { "Name" = "${var.prefix_name_tag}spoke1-ec2messages-endpoint"})
}
...
resource "aws_vpc_endpoint" "spoke1_ssmmessages" {
  ...
  private_dns_enabled = true
  security_group_ids = [module.spoke1_vpc.security_group_ids["web-server-sg"]]
  tags                = merge(var.global_tags, { "Name" = "${var.prefix_name_tag}spoke1-ssmmessages-endpoint"})
}
...
resource "aws_vpc_endpoint" "spoke2_ssm" {
  ...
  tags                = merge(var.global_tags, { "Name" = "${var.prefix_name_tag}spoke2-ssm-endpoint"})
}
...
resource "aws_vpc_endpoint" "spoke2_kms" {
  ...
  private_dns_enabled = true
  security_group_ids = [module.spoke2_vpc.security_group_ids["web-server-sg"]]
  tags                = merge(var.global_tags, { "Name" = "${var.prefix_name_tag}spoke2-kms-endpoint"})
}
...
resource "aws_vpc_endpoint" "spoke2_ec2messages" {
  ...
  vpc_endpoint_type = "Interface"
  security_group_ids = [module.spoke2_vpc.security_group_ids["web-server-sg"]]
  tags                = merge(var.global_tags, { "Name" = "${var.prefix_name_tag}spoke2-ec2messages-endpoint"})
}
...
resource "aws_vpc_endpoint" "spoke2_ssmmessages" {
  ...
  private_dns_enabled = true
  security_group_ids = [module.spoke2_vpc.security_group_ids["web-server-sg"]]
  tags                = merge(var.global_tags, { "Name" = "${var.prefix_name_tag}spoke2-ssmmessages-endpoint"})
}
